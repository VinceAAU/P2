...
export { savePendingQueue, addCustomerQueue, removeCustomerQueue, getUserQueueHead, getTaskQueueHead, pendingQueueToFinishedQueue, loadPendingQueue, debugging };
...
async function loadPendingQueue(){      //loads the pending queue from the server
    if (existsSync("master/autoGeneratedFiles/pendingQueue.csv")){  //Checks if a pendingQueue.csv exists
        ...
        userPendingQueue = rowOne.split(',');
        taskPendingQueue= rowTwo.split(',');
...
async function debugging(){
    await loadPendingQueue();
    //console.log(await getTaskQueueHead());
    //console.log(await getUserQueueHead());
}
...
import fs2 from "fs";
import { existsSync, createWriteStream, open} from "fs";
export {
  assignWorkToWorker,
  enqueueTask,
  ...
  if (fileLoad == false) {
    if (allTasks == null || allTasks.length === 0) {
      ...
      if (allTasks !== null) {
        ...
        bucketOffsets = getBucketFileOffsets(allTasks);
      } else {
        qHead = 0;
        qTail = 0;
      }
...
async function storeSortedBuckets(bucket) {
  ...
  if (!fileCreated) {
    ...
    outputFilePath = `master/autogeneratedFiles/csvFiles/${uniqueFileName}`;
  ...
  console.log("Writing bucket: " + indexForSortedBucket + " at offset: " + bucketOffsets[indexForSortedBucket]);
  writeBucketToFile(bucketOffsets[indexForSortedBucket], bucket, outputFilePath);
...
function getCharsForCSV(inputArray) {
  ...
  console.log("Calculated offset: " + numberOfDigits);
...
function writeBucketToFile(offset, bucket, filePath) {
  console.log("Opening file!!!"); // there may or may not be issues with constantly opening and closing the file
  fs2.open(filePath, 'r+', (err, fd) => {
      if (err) throw err;

      const csvString = bucket.join(',') + '\n';
      const buffer = Buffer.from(csvString);
      fs2.write(fd, buffer, 0, buffer.length, offset, (err) => {
          if (err) throw err;

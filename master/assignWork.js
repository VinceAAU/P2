...
async function bucketConcatenate() {
  ...
  let timeConcat = new Date().getTime();

  const writeStream = createWriteStream(`master/autogeneratedFiles/csvFiles/${uniqueFileName}`);
  let i = 0;
  for (let element of sortedBuckets) {
    console.log(i);
    if (!writeStream.write(`${element},`)) {
      console.log("Waiting for buffer to drain, whatever that means");
      await new Promise(resolve => writeStream.once('drain', resolve));
    }
    i++
  }

  console.log("Concat took: " + ((new Date().getTime() - timeConcat)/1000) + " seconds" );
  
  writeStream.end();

  console.log("Total time is : " + ((new Date().getTime() - startTime)/1000) + " seconds" );
